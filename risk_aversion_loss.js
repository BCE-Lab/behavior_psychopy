...
let expName = 'risk_aversion_loss';  // from the Builder filename that created this script
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'Image/RAL.PNG', 'path': 'Image/RAL.PNG'}
  ]
});
...
async function experimentInit() {
  ...
  inst_img = new visual.ImageStim({
    ...
    image : 'Image/RAL.PNG', mask : undefined,
...
function PayOffRoutineBegin(snapshot) {
  return async function () {
    ...
    txt_left = (((s_payoff1.toString() + "\uc6d0") + "\n") + "100%");
    txt_right = ((((("-" + s_payoff2.toString()) + "\uc6d0") + "\n") + percentage[current].toString()) + "%");
    console.log("s_payoff : ", s_payoff2);
