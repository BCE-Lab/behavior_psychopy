...
expName = 'risk_aversion'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\OneDrive - pusan.ac.kr\\바탕 화면\\연구\\2021 행동검사 웹플랫폼\\Scale\\플랫폼 버전관리\\행동검사\\ver1.0\\risk_aversion\\risk_loss\\risk_aversion_loss.py',
...
inst_img = visual.ImageStim(
    win=win,
    name='inst_img', 
    image='Image/RAL.PNG', mask=None, anchor='center',
...
# Limit
OUL = [6*x for x in payoff_start]    # Outer Upper limit
IUL = [6*x for x in payoff_start]    # Inner Upper limit
...
for thisTrial in Trials:
    ...
    # text setting(gain)
    txt_left = str(s_payoff1)+"원"+"\n"+"100%"
    txt_right = "-"+str(s_payoff2)+"원"+"\n"+str(percentage[current])+"%"
    print("s_payoff : ", s_payoff2)
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    PayOffComponents = [Choice_Left, Choice_Right, Choice_Cursor, Text_Left, Text_Right, mouse]
    ...
    print(payoff_gamble2)
    # choosing Left
    if mouse.clicked_name[-1]=="Choice_Right":  
        choice = -1
    ...
    # choosing Right
    elif mouse.clicked_name[-1]=="Choice_Left": 
        choice = 1
        # Adjusting payoff: Increasing payoff(lower limit)
        if (payoff_gamble2 > IUL[current]):
    ...
    Trials.addData('Text_Right.started', Text_Right.tStartRefresh)
    Trials.addData('Text_Right.stopped', Text_Right.tStopRefresh)
