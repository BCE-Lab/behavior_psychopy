...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\OneDrive - pusan.ac.kr\\바탕 화면\\연구\\2021 행동검사 웹플랫폼\\Scale\\플랫폼 버전관리\\행동검사\\ver1.0\\delaydiscounting\\delay.py',
...
inst_img = visual.ImageStim(
    win=win,
    name='inst_img', 
    image='Image/DD.PNG', mask=None, anchor='center',
...
# Limit
OUL = [10*x for x in payoff_start]    # Outer Upper limit
IUL = [10*x for x in payoff_start]    # Inner Upper limit
ILL = [1*x for x in payoff_start]    # Inner Lower limit
OLL = [1*x for x in payoff_start]    # Outer Lower limit
...
for thisTrial in Trials:
    ...
    # text setting(gain)
    txt_left = "+"+str("50000")+"원"+"\n"+"지금"
    txt_right = "+"+str(s_payoff2)+"원"+"\n"+str(days[current])+"일 뒤"
